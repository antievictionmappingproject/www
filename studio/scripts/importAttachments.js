...
async function run() {
      ...
      queue.add(async () => {
        ...
        try {
          const readStream = got
            ...
            .on("error", () => {
              readStream.destroy(`Attachment ${id}: Failed to get`);
            });
          const document = await client.assets.upload("image", readStream, {
            filename: item.title,
          });
  ...
  queue.on("idle", () => {
    console.log("Idle");
    fsp.writeFile(attachmentsFile, JSON.stringify(Array.from(ids)));
  });
